...
def main2():
    # 获取已标注的数据
    futureData, labelData = get_csv("./真假粉判断标注.csv")
    # 训练
    clf = train(futureData, labelData)
    # 获取待预测的数据
    file_dir = r'./临时数据/'
        ...
        for file in files:
            ...
            with open(file_dir + file, 'r', encoding='utf-8') as input_file:
                ...
                for line in f_csv:
                    ...
                    # print(line)
                    print(line)
                    temp.append(int(line[3].strip()))
                    temp.append(int(line[4].strip()))
                    temp.append(int(line[5].strip()))
                    temp.append(int(line[6].strip()))
                    temp.append(int(line[7].strip()))
                    temp.append(int(line[8].strip()))
                    temp.append(int(line[9].strip()))
                    predictData.append(temp)
            ...
            # 写出
    print(resultdata)
    with open("./临时数据/result.csv", 'w', newline='', encoding='utf-8') as file:
